...
class ESearch(object):
    def __init__(self, index_name, doc_type_name, \
        ...
        self.bert = BertClient(port=86500, port_out=86501, show_server_config=True, timeout=1000000)  # 导入bert句子向量
    ...
    def load_data_from_file(self, input_file, overwrite=False):
        ...
        for i in range(len(df)):
            self.make_action(df["knowledge_q"][i], actions, i)
    ...
    def split_word(self, query):
        '''
        jieba分词器：返回分词、关键词、句向量
        '''
        splwords = "/".join(jieba.cut(query))
        embeding = self.bert.encode(list(query))
        keywords = " ".join(jieba.analyse.extract_tags(query,topK=6))
    ...
    def query(self, sentences, n=5):
        splwords = "/".join(jieba.cut(sentences))
        keywords = jieba.analyse.extract_tags(sentences,topK=6)
        keywords = " ".join(keywords)
        embeding = self.bert.encode(list(sentences)).tolist()[0]
...
if __name__ == "__main__":
    ...
    # 查询
    print()
    esearch.query('如何学好编程？')

    #esearch.query('参加网上的理财培训靠谱吗？')
